...
namespace UseApi.Controllers
{
    public class WellTestController : Controller
    {
        ...
        public WellTestController(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            HttpResponseMessage response = await httpClient.GetAsync("/ApiGateway/Well/WellTest");
            bool showTable = false;
            if (response.IsSuccessStatusCode)
            {
                var json = await response.Content.ReadAsStringAsync();
                var _list = JsonConvert.DeserializeObject<List<WellTest>>(json);
                showTable = true;
        ...
        public async Task<IActionResult> Create()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (_list.Count > 0)
                {
                    int maxId = _list.Max(wellTest => wellTest.Id);
        ...
        [HttpPost]
        public async Task<IActionResult> Create(WellTest model)
        {
            if (ModelState.IsValid)
            {
                ...
                HttpResponseMessage response = await httpClient.PostAsync("/ApiGateway/Well/WellTest", content);
                if (response.IsSuccessStatusCode)
                {
        ...
        {
            var httpClient = _httpClientFactory.CreateClient("API_OCELOT");
            var deleteUrl = $"/ApiGateway/Well/WellTest/{id}";
